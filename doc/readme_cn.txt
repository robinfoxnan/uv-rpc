...
void testServerPing()
{
	...
	// 1）设置收消息解析与分发类，可以需要自己定义自己的分发类，重写onMessageParse()函数即可；
	...
	{
		pakcetCount++;
		printf("packet count:%ju\n", pakcetCount);
	};*/

	// 3）设置发送前消息编码器，编码器和解码器需要根据RPC传递的内容，自己定义
	std::shared_ptr<PingEncode> encoder = std::make_shared<PingEncode>();
	GlobalConfig::setEncoder(encoder);
	SocketAddr address("0.0.0.0", 80);
	TcpServer server;
	server.bindAndListen(address, 128);
	server.start();
